 ...
 * @param <T>
 */
public class HttpVolleyStrRequestCanCache<T> {
	private StringRequestCallBack mListener;
	...
	public HttpVolleyStrRequestCanCache() {

	}

	public HttpVolleyStrRequestCanCache(Activity mAct) {
		this.mAct = mAct;
	}

	public HttpVolleyStrRequestCanCache(Activity mAct, boolean isCache) {
	...
	private Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			mListener.onStrResponse((String) t);
		}
	};
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void HttpVolleyRequestStrGet(String url,
			StringRequestCallBack listener, Context context) {
		...
		if (!CheckNetworkType.checkNetworkState(context)) {
			...
			if (queue.getCache().get(url) != null) {
				...
				if (mListener != null) {
					// 返回缓存数据
					mListener.onStrResponse(cachedResponse);
				}
			} else {
		...
		StrCacheRequest request = new StrCacheRequest(Method.GET, url,
				ErrorListener(), params, SuccessListener());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void HttpVolleyRequestStrPost(String url,
			HashMap<String, String> params, StringRequestCallBack listener,
			Context context) {
		mListener = listener;
		this.urlKey = url;
		...
		StrCacheRequest request = new StrCacheRequest(Method.POST, url,
				ErrorListener(), params, SuccessListener());
		RequestManager.getRequestQueue().add(request);
		...
		return new Listener<T>() {
			...
			public void onResponse(T response) {
				...
				if (isCache) {
					if (!isExit) {
						if (mListener != null) {
							mListener.onStrResponse((String) response);
						}
					}
					if (method == Method.GET) {
						App.getInstance()
								.getCache()
								.put(getCacheKey(urlKey), (String) response,
										DataCache.TIME_DAY);
					} else {
						App.getInstance()
								.getCache()
								.put(getPostCacheKey(urlKey, params),
										(String) response, DataCache.TIME_DAY);
					}
	...
	private ErrorListener ErrorListener() {
		return new ErrorListener() {
			...
			public void onErrorResponse(VolleyError error) {
				...
				if (mListener != null)
					mListener.onStrResponseError(error);
			}
		};
	}
